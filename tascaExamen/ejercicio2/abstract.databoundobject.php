...
abstract class DataBoundObject {
   ...
   public function __construct(PDO $objPDO,$logf = NULL, $classNameF = NULL, $logp = NULL, $classNameP = NULL, $id = NULL ) {
      echo '<br> log file construct -----:';
      
      $this->strTableName = $this->DefineTableName();
      $this->arRelationMap = $this->DefineRelationMap();
      $this->objPDO = $objPDO;
      $this->blIsLoaded = false;
      echo "<br>---------".$classNameP.'<br>';
      
      if (isset($logf)) {
         $this->logfile = $classNameF::getInstance();
     } else {
         echo '<br> no log file';
     }
     if (isset($logp)) {
      $this->logpgsql = $classNameP::getInstance();
  } else {
      echo '<br> no pgsql file';
  }
  if (isset($classNameF)) {
   $this->classNameFile = $classNameF;
} else {
   echo '<br> no log file';
}
if (isset($classNameP)) {
   $this->classNamePgsql = $classNameP;
} else {
   echo '<br>no pgsql file';
} 
